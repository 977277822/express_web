		...
		var validatorFn = function() {
			...
			this.defaultShowError = function(elem, errorMessages) {
				var $elem = angular.element(elem),
					...
					$wrap = $elem.parent().find(".w5c-error-wrap");
				...
				var $next = $elem.next();
				if ($wrap.length > 0) {
					$wrap.empty().append('<span class="w5c-error">' + errorMessages[0] + '</span>');
				} else if (!$next || !$next.hasClass("w5c-error")) {
			...
			this.defaultRemoveError = function(elem) {
				...
				var $next = $elem.next();
				if ($wrap.find(".w5c-error-wrap").length > 0) {
					$wrap.find(".w5c-error").remove();
				} else if ($next.hasClass && $next.hasClass("w5c-error")) {
					$next.remove();
				}
